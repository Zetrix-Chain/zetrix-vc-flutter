...
import com.zetrix.bbs.signatures.BlindedKeyPair;
import com.zetrix.bbs.signatures.BlindCommitmentContext;
import com.zetrix.bbs.signatures.ProofMessage;

public class Bbs {

    static {
        // System.loadLibrary("bbs_signatures_jni");
    ...
    private static native int bls_generate_blinded_g1_key(byte[] seed, byte[] public_key, byte[] secret_key,
            byte[] blinding_factor);

    private static native int bls_generate_blinded_g2_key(byte[] seed, byte[] public_key, byte[] secret_key,
            byte[] blinding_factor);
    ...
    private static native int bbs_unblind_signature(byte[] blind_signature, byte[] blinding_factor,
            byte[] unblind_signature);
    ...
    private static native int bbs_create_proof_context_add_proof_message_bytes(long handle, byte[] message, int xtype,
            byte[] blinding_factor);
    ...
    public static BlindCommitmentContext blindCommitment(byte[] public_key, Map<Integer, byte[]> messages, byte[] nonce)
            throws Exception {
    ...
    public static byte[] blindSign(byte[] secret_key, byte[] public_key, byte[] commitment,
            Map<Integer, byte[]> messages) throws Exception {
    ...
    public static byte[] createProof(byte[] publicKey, byte[] nonce, byte[] signature, ProofMessage[] messages)
            throws Exception {
        ...
        for (ProofMessage message : messages) {
            if (0 != bbs_create_proof_context_add_proof_message_bytes(handle, message.message, message.type,
                    message.blinding_factor)) {
    ...
    public static byte[] blsCreateProof(byte[] public_key, byte[] nonce, byte[] signature, ProofMessage[] messages)
            throws Exception {
        byte[] bbs_public_key = Bbs.blsPublicToBbsPublicKey(public_key, messages.length);
        long handle = bbs_create_proof_context_init();
        if (0 == handle) {
    ...
    public static boolean verifyProof(byte[] public_key, byte[] proof, byte[] nonce, byte[][] messages)
            throws Exception {
    ...
    public static boolean blsVerifyProof(byte[] public_key, byte[] proof, byte[] nonce, byte[][] revealed_messages)
            throws Exception {
    ...
    public static String createProof(String[] messages) {
        // TODO: Call into actual JNI native function
        return "mocked-proof-value-from-Bbs"; // Replace later
    }
}